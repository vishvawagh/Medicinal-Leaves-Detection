...
function Home() {
  ...
  const fetchVotes = async (plantName) => {
    try {
      const res = await axios.get(`http://127.0.0.1:5000/votes/${encodeURIComponent(plantName)}`);
      setVotes(res.data);
    } catch (err) {
  ...
  const sendVote = async (type) => {
    ...
    try {
      await axios.post("http://127.0.0.1:5000/vote", {
  ...
  const onFileUpload = async () => {
    ...
    try {
      ...
      if (response.data && Object.keys(response.data).length > 0) {
        ...
        await axios.post("http://127.0.0.1:5000/update-history", response.data);
  ...
  const clearHistory = async () => {
    try {
      await axios.post("http://127.0.0.1:5000/clear-history");
      setHistory([]);
    } catch (err) {
